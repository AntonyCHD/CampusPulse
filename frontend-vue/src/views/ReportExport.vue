...
<template>
  <div class="report-export">
    <div class="page-header">
      ...
      <el-button v-if="report && !loading" :icon="Download" type="primary" @click="exportMarkdown" class="export-btn">
        导出 Markdown
      </el-button>
    </div>
    ...
    <template v-if="eventId && loading">
      <SkeletonPresets section="report" />
    </template>
...
<style scoped>
.report-export { padding: 16px 24px; }
...
.selector-bar { background: white; border: 2px dashed #DBEAFE; border-radius: 12px; padding: 32px; text-align: center; }
...
.section-card-header { font-family: var(--font-heading); font-size: 14px; font-weight: 600; color: #1E3A8A; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #f1f5f9; }
.mono-text { font-family: var(--font-heading); }
.kpi-card { display: flex; flex-direction: column; gap: 4px; padding: 12px; background: #f8fafc; border-radius: 8px; }
.kpi-label { font-size: 12px; color: #64748B; }
.kpi-value { font-family: var(--font-heading); font-size: 20px; font-weight: 700; color: #1E3A8A; }
.evolution-line { display: flex; align-items: center; gap: 8px; padding: 10px 14px; background: #f8fafc; border-radius: 8px; }
.ev-label { font-size: 13px; color: #64748B; flex-shrink: 0; }
.ev-path { font-size: 14px; color: #1E3A8A; font-weight: 500; }
.finding-card { padding: 16px; background: #f8fafc; border-radius: 8px; text-align: center; }
