...
* **前端 Demo：Vue 3 + TypeScript + Element Plus**（早期方案曾建议 Streamlit，当前仓库实现以 `frontend-vue/` 为准）
* **后端服务：FastAPI**
* **数据存储：SQLite / DuckDB**
...
* **可视化：ECharts / vis-network / Element Plus 组件**
* **部署：本地运行 + Docker Compose 兜底**

FastAPI 适合作为后端 API 框架，因为它基于 Python 类型提示构建 API，并能自动生成交互式 API 文档；这对调试接口、验证后端不是脚本拼接很有帮助。([FastAPI][1]) 当前可运行前端已切换为 Vue 3 + TypeScript + Element Plus，Streamlit 仅作为早期快速 Demo 方案背景保留。Pydantic 则负责数据校验，它基于 Python 类型提示定义数据结构，非常适合把“大模型输出 JSON”变成可验证对象。([Pydantic][3])

### 总体架构图

```text
┌────────────────────────────────────────────────────────────┐
│                    Vue 3 / Element Plus Demo 前端            │
...
| M9 | 可视化 Demo | 必须 | Vue 3 展示事件列表、单事件分析、关键评论、对比结果、缓存报告 | 后端 API JSON | UI 页面 | 无需改代码即可切换事件和缓存模式 |
...
│
├── frontend-vue/
│   ├── package.json                 # Vue 3 前端依赖与脚本
│   ├── src/
│   │   ├── api/client.ts            # 后端 API 客户端与类型
│   │   ├── views/EventList.vue      # 事件总览
│   │   ├── views/EventAnalysis.vue  # 单事件分析
│   │   ├── views/Comparison.vue     # 对比实验
│   │   ├── views/CommentGraph.vue   # 评论图谱
│   │   └── views/ReportExport.vue   # 报告导出
│
├── notebooks/
